...
1. Initialize a transaction:This step is where the producer registers a transaction using `transactional.id`.
   ...
   check [Internal Link: recommended configurations](producer_settings.html#idempotent-producer-and-transactional-producer) for how to set `transactional.id`.
...
![](http://client/../_images/transactional_producer.png)

For more details on [External Link: Transactions
