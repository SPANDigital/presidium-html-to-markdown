...
#### Creating an encryption context[¶]({{< ref "\#creating-an-encryption-context" >}})
...
#### Grant a consumer decryption access[¶]({{< ref "\#grant-a-consumer-decryption-access" >}})
...
#### Revoke a consumer decryption access[¶]({{< ref "\#revoke-a-consumer-decryption-access" >}})
...
**This is done to allow all consumers to be configured with the rolled private states or pick them up automatically from [Kaffe]({{< ref "kaffe" >}}).**
**You should confirm the revocation only when all consumers are up-to-date with the new epoch.**

#### Configuring producers and consumers[¶]({{< ref "\#configuring-producers-and-consumers" >}})
...
We **strongly recommend** enabling [Kaffe configuration injection]({{< ref "default_config_injection" >}}) if enabling encryption or/and decryption.
...
##### What if producer or consumer is required to use a different encryption context for multiple topics?[¶]({{< ref "\#what-if-producer-or-consumer-is-required-to-use-a-different-encryption-context-for-multiple-topics" >}})
...
##### I don’t trust ASE Kafka team to persist the encrypted context master secret. Can I persist encrypted context master secret locally on disk?[¶]({{< ref "\#i-don-t-trust-ase-kafka-team-to-persist-the-encrypted-context-master-secret-can-i-persist-encrypted-context-master-secret-locally-on-disk" >}})
