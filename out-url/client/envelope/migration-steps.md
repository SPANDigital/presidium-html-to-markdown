...
- Use kafka-pie-client version 0.15 or newer - [Internal Link: check the current version recommended for production](../releases/index.html#recommended-release).
...
### PieEnvelope Deprecation and Migration details[Internal Link: ¶](\#pieenvelope-deprecation-and-migration-details)

The ACI Kafka team has deprecated both the `PieEnvelope` Java object and
the [Internal Link: old binary format](#old-binary-format).
...
All these variants support both the [Internal Link: new binary
format](#new-binary-format-introduced-in-kafka-pie-client-0-15) and the
[Internal Link: old binary format](#old-binary-format). The [Internal Link: pie.fmt record
...
defaults to true, using the [Internal Link: old binary format](#old-binary-format).

The new producer - created with `createProducer` \- chooses the
appropriate message format based on the value of the
`pie.queue.envelope.serializer.use.payload.envelope` property, which
defaults to false, using the [Internal Link: new binary
...
serialized format. When no [Internal Link: pie.fmt record header](#two-binary-formats)
...
> property, which defaults to true and indicates that the [Internal Link: old
...
> defaults to false and indicates that the [Internal Link: new binary
...
names of these headers are `pie.ext.X.Y`. `X` is the [Internal Link: organization
id](#organization-id) and `Y` is the [Internal Link: extension id](#extension-id).

A more detailed discussion of this topic can be found in
[External Link: QTIP-028](https://github.pie.apple.com/pie/kafka-tools/blob/develop/improvement-proposals/028-envelope-deprecation.md).

### Envelope motivation[Internal Link: ¶](\#envelope-motivation)
...
### Two binary formats[Internal Link: ¶](\#two-binary-formats)
...
### New binary format (introduced in kafka-pie-client 0.15)[Internal Link: ¶](\#new-binary-format-introduced-in-kafka-pie-client-0-15)
...
### Old binary format[Internal Link: ¶](\#old-binary-format)
...
The legacy PieEnvelope binary format uses the standard [External Link: Kafka
serialization](https://github.pie.apple.com/pie/kafka-tools/blob/develop/pie-envelope/src/main/java/com/apple/pie/queue/kafka/envelope/EnvelopeDeserializer.java#L40-L74)
...
[External Link: EnvelopeProtocol](https://github.pie.apple.com/pie/kafka-tools/blob/develop/pie-envelope/src/main/java/com/apple/pie/queue/kafka/envelope/internal/EnvelopeProtocol.java).
...
### Organization id[Internal Link: ¶](\#organization-id)
...
### Extension id[Internal Link: ¶](\#extension-id)

Extension ids need only be unique within an organization, so it is up to
the organization to avoid conflicts.

### Using envelope extensions in the client[Internal Link: ¶](\#using-envelope-extensions-in-the-client)

To be able to use an envelope extension you will need to give the client
a subclass of
[External Link: com.apple.pie.queue.kafka.envelope.extensions.ExtensionSerDeserializer](https://github.pie.apple.com/pie/kafka-tools/blob/develop/pie-envelope/src/main/java/com/apple/pie/queue/kafka/envelope/extensions/ExtensionSerDeserializer.java).
...
[External Link: getExtension(ExtensionKey)](https://github.pie.apple.com/pie/kafka-tools/blob/develop/pie-envelope/src/main/java/com/apple/pie/queue/kafka/envelope/PieEnvelope.java)
...
### Chaining Serialization[Internal Link: ¶](\#chaining-serialization)
...
![](http://client/../_images/serializer_chain.png)
...
### 0-copy optimization (optional)[Internal Link: ¶](\#copy-optimization-optional)
...
### Advice on writing good extensions[Internal Link: ¶](\#advice-on-writing-good-extensions)
...
### Existing extensions[Internal Link: ¶](\#existing-extensions)
...
- [External Link: avro
  ...
  with
  [External Link: schema-store](https://github.pie.apple.com/amp-content-infra/schema-store)
...
### Kafka-extensions repository[Internal Link: ¶](\#kafka-extensions-repository)

[External Link: Kafka extensions](https://github.pie.apple.com/pie/kafka-extensions) is
