...
To authorize a Kubernetes namespace for ACI Kafka self-service, [Internal Link: customer groups](../concepts.html#customer-group) must first be decorated with the Kubernetes namespace name, as well as the Kubernetes cluster name.
Refer to the relevant section in [Internal Link: Public API](public_api.html#kube) or [Internal Link: ACI Kafka CLI](kafka_cli.html#kube) for instructions.
