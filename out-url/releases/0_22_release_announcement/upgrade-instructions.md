...
The ACI Kafka 0.22 client is available on [External Link: Artifactory](https://artifacts.apple.com/pie-release-local/com/apple/pie/queue/kafka-pie-client).
...
- The default value for `acks` is set to `-1` (all). See [External Link: KIP-697](https://cwiki.apache.org/confluence/display/KAFKA/KIP-679%3A+Producer+will+enable+the+strongest+delivery+guarantee+by+default) for more details.
- The default value for `enable.idempotence` is set to `true` for ACI Kafka versions >= 0.22.13 if
  `acks=-1`, `max.in.flight.requests.per.connection <= 5` and `retries > 0`. See [External Link: KIP-697](https://cwiki.apache.org/confluence/display/KAFKA/KIP-679%3A+Producer+will+enable+the+strongest+delivery+guarantee+by+default) and
  [External Link: KAFKA-13598](https://issues.apache.org/jira/browse/KAFKA-13598) for more details.
- The default value for the consumer configuration session.timeout.ms was increased from 10s to 45s.
  See [External Link: KIP-735](https://cwiki.apache.org/confluence/display/KAFKA/KIP-735%3A+Increase+default+consumer+session+timeout) for more details.
- The `Producer#sendOffsetsToTransaction(Map offsets, String consumerGroupId)` method has been deprecated.
  ...
  See [External Link: KIP-732](https://cwiki.apache.org/confluence/x/zJONCg) for more details.
- A number of deprecated classes, methods and configurations have been removed:
  ...
  - The default `partition.assignment.strategy` is changed to “\[ `RangeAssignor`, `CooperativeStickyAssignor`\]”,
    ...
    [External Link: here](https://cwiki.apache.org/confluence/display/KAFKA/KIP-429:+Kafka+Consumer+Incremental+Rebalance+Protocol#KIP429:KafkaConsumerIncrementalRebalanceProtocol-Consumer).
    for more detail.
...
For general overview on what’s new in Kafka 3.0.0 you might be interested in [External Link: this post](https://blogs.apache.org/kafka/entry/what-s-new-in-apache6) in the Apache Kafka
blog.

For additional changes and more details please consult the notable changes for version
[External Link: 3.0](https://kafka.apache.org/documentation.html#upgrade_300_notable) in the Apache Kafka documentation or the [External Link: release notes for Kafka 3.0](https://kafka.apache.org/downloads.html#3.0.0).
