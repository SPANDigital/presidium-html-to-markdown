...
### How to find the exact exception from Kafka?[Internal Link: ¶](\#how-to-find-the-exact-exception-from-kafka)
...
#### Impact[Internal Link: ¶](\#impact)

Client is interrupted and it is taking longer to track down the root cause.

#### Causes and Solutions[Internal Link: ¶](\#causes-and-solutions)
...
- Trace the error back to Kafka’s logs and exceptions. These will be prefixed with:
  ...
  - `com.apple.pie.queue`: If client has errors from `com.apple.pie.queue` package please check the connectivity with [Internal Link: Kaffe endpoints](../client/kaffe.html#kaffe-endpoints) using `nc` bash command.
...
If you still suspecting Kafka but don’t have enough evidences please check section [Internal Link: Is my cluster healthy?](#is-my-cluster-healthy).

### Is my cluster healthy?[Internal Link: ¶](\#is-my-cluster-healthy)

#### Description[Internal Link: ¶](\#id1)
...
#### Causes and Solutions[Internal Link: ¶](\#id3)
...
Most Kafka [Internal Link: errors are informative](#too-many-logs-saying-broker-may-not-be-available), and the client can recover from them without any intervention.

_Solution:_

- [Internal Link: Check that the Kafka cluster is healthy](general.html#checking-cluster-health).
- Produce and consume experience are monitored through [Internal Link: Synthetic Metrics](../monitoring/broker_metrics.html#synthetic-metrics).

### Too many logs saying `Broker may not be available`[Internal Link: ¶](\#too-many-logs-saying-broker-may-not-be-available)

#### Description[Internal Link: ¶](\#id4)
...
#### Causes and Solutions[Internal Link: ¶](\#id6)

- Make sure to [Internal Link: test the connectivity](connectivity.html) with the brokers before investigating any causes.
- [Internal Link: Check that the Kafka cluster is healthy](general.html#checking-cluster-health).
...
1. a temporary issue, meaning that the client is recovering on the next retry of the consumer [Internal Link: poll](../client/consumer_client.html#poll) function, or a producer successfully [External Link: retried sending the batch](https://docs.aci.apple.com/kafka/client/producer_client.html#sender-thread).
   Or that the [Internal Link: cluster is healthy](general.html#checking-cluster-health).
...
- If this is a producer client, make sure `retries` config is set to a higher number so the client will not give up on the batch before the temporary network error resolves itself.
  Check our [Internal Link: recommendation for best delivery guarantee](../client/producer_settings.html#best-delivery-guarantee) for more details regarding how to choose an appropriate value.
