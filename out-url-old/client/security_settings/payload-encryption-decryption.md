...
In addition, the context owners must install `kafkacli` from [Homebrew](https://github.pie.apple.com/homebrew/brew) using:
...
#### Creating an encryption context [¶](#creating-an-encryption-context "Link to this heading")
...
Please, note down the context master secret key that’s printed in the console and save it securely (i.e., [Whisper](https://whisper.corp.apple.com/), [sops](https://github.com/getsops/sops), macOS Keychain or 1Password).
...
#### Grant a consumer decryption access [¶](#grant-a-consumer-decryption-access "Link to this heading")
...
#### Revoke a consumer decryption access [¶](#revoke-a-consumer-decryption-access "Link to this heading")
...
**This is done to allow all consumers to be configured with the rolled private states or pick them up automatically from [Kaffe]({{< ref "client/kaffe" >}}).**
**You should confirm the revocation only when all consumers are up-to-date with the new epoch.**

#### Configuring producers and consumers [¶](#configuring-producers-and-consumers "Link to this heading")
...
We **strongly recommend** enabling [Kaffe configuration injection]({{< ref "client/default_config_injection" >}}) if enabling encryption or/and decryption.
...
Should you have any questions, do not hesitate to [reach out to us]({{< ref "contact" >}}).

#### FAQ [¶](#faq "Link to this heading")

##### What if producer or consumer is required to use a different encryption context for multiple topics? [¶](#what-if-producer-or-consumer-is-required-to-use-a-different-encryption-context-for-multiple-topics "Link to this heading")
...
##### I don’t trust ASE Kafka team to persist the encrypted context master secret. Can I persist encrypted context master secret locally on disk? [¶](#i-don-t-trust-ase-kafka-team-to-persist-the-encrypted-context-master-secret-can-i-persist-encrypted-context-master-secret-locally-on-disk "Link to this heading")
