...
- **When to use Exactly-once over At-Least-Once**If your use-case requires avoiding processing a record multiple
  ...
  [Idempotent Producer]({{%baseurl%}}/producer_client.html#exactly-once)).
...
- **When to use Transactional Producer**You may need to consider [Transactions Producer]({{%baseurl%}}/producer_client.html#transactional) in
...
Any ACI-Kafka client identity with a [client identity topic access]({{%baseurl%}}/../concepts.html#client-identity-topic-access) set to `PRODUCE`
is authorized to create [transactions]({{%baseurl%}}/producer_client.html#transactional) with `transactional.id`
prefixed with their [ACI-Kafka client identity]({{%baseurl%}}/../concepts.html#client-identities).

### Common problems with EOS producers [¶](#common-problems-with-eos-producers "Link to this heading")

#### Misconfigured or rouge idempotent producers will cause Kafka clusters to run out-of-memory (OOM) [¶](#misconfigured-or-rouge-idempotent-producers-will-cause-kafka-clusters-to-run-out-of-memory-oom "Link to this heading")
