...
You can also use the [**kcurl**]({{%baseurl%}}/https://docs.aci.apple.com/kafka/self-service/public_api.html#kcurl) example if you don’t want to repeat the code for each configuration.
...
### Setting up IAS client environment variables [¶](#setting-up-ias-client-environment-variables "Link to this heading")

To generate ID tokens, you must set up your IAS client credentials using the steps below:

1. Open a new terminal session and follow the [Installing IAS CLI](https://docs.aci.apple.com/ias/docs/IAS_installation.html) instructions to install, update, and verify the command line interface (CLI).
...
### Generating a JSON Web Token [¶](#generating-a-json-web-token "Link to this heading")

You can generate a new ID token for the authorization header using the [IAS API](https://docs.aci.apple.com/ias/tutorials/client_credentials/index.html):
...
If you have trouble running the commands above, see the [IAS documentation](https://docs.aci.apple.com/ias/docs/creating_an_IAS_application.html).

### kcurl [¶](#kcurl "Link to this heading")
...
You must install [`jq`](https://stedolan.github.io/jq/): `brew install jq` for macOS before defining the function.
